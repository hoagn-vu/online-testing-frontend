    ...
    data
      ...
      .forEach((item, index) => {
        if (!chapterSummary[item.chapter]) {
          const levels = Array.isArray(item.levels) ? item.levels : [item]; // Xử lý nếu không có levels
          chapterSummary[item.chapter] = {
            chapter: item.chapter,
            // Nếu đang edit -> lấy trực tiếp questionCount, nếu đang thêm mới -> 0
            totalSelected: levels.reduce((sum, level) => sum + (level.questionCount ?? 0), 0),
            // total có thể undefined => fallback = questionCount
            totalQuestions: levels.reduce((sum, level) => sum + (level.total ?? 0), 0),
            // Nếu đang edit -> lấy luôn score, nếu chưa có thì mặc định 0
            score: levels.reduce((sum, level) => sum + (level.score ?? 0), 0),
            originalIndex: index, // Lưu index gốc để ánh xạ
          };
        } else {
          const levels = Array.isArray(item.levels) ? item.levels : [item];
          chapterSummary[item.chapter].totalSelected += levels.reduce((sum, level) => sum + (level.questionCount ?? 0), 0);
          chapterSummary[item.chapter].totalQuestions += levels.reduce((sum, level) => sum + (level.total ?? 0), 0);
          chapterSummary[item.chapter].score += levels.reduce((sum, level) => sum + (level.score ?? 0), 0);
        }
      });
